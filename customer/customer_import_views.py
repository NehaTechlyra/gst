"""
Customer Import Wizard – customer_import_views.py
Mapped exactly to your Customer model fields.
Place in Customer/customer_import_views.py  (or wherever your customer app lives)
"""
...
IMPORT_FIELDS = [
    # Basic Info
    ("customer_type",    "Customer Type (individual/company)",   False, "basic",    "individual or company"),
    ...
    ("is_vendor",        "Is Vendor (true/false)",               False, "basic",    ""),
...
def _validate_rows(rows, mapping):
    ...
    for row in rows:
        ...
        ct = mapped.get("customer_type", "").lower()
        if ct and ct not in ("individual", "company"):
            errors.append(f"Customer Type must be 'individual' or 'company' (got: {ct!r})")
...
def import_customers_step1(request, company_code):
    if request.method == "POST":
        uploaded = request.FILES.get("import_file")
        if not uploaded:
            messages.error(request, "Please select a file.")
            return render(request, "customer/customer_import_step1.html", {"company_code": company_code})

        ext = uploaded.name.rsplit(".", 1)[-1].lower()
        if ext not in ("csv", "tsv", "xls", "xlsx"):
            messages.error(request, "Only CSV, TSV, XLS, or XLSX files are supported.")
            return render(request, "customercustomer_import_step1.html", {"company_code": company_code})
        ...
        return redirect("import_customers_step2", company_code=company_code)
...
def import_customers_step2(request, company_code):
    headers = request.session.get("cust_imp_headers")
    if not headers:
        return redirect("import_customers_step1", company_code=company_code)
    ...
    if request.method == "POST":
        ...
        if not any(v == "email" for v in mapping.values()):
            ...
            return redirect("import_customers_step3", company_code=company_code)

    fields_by_section = get_fields_by_section()

    return render(request, "customer/customer_import_step2.html", {
...
def import_customers_step3(request, company_code):
    headers   = request.session.get("cust_imp_headers")
    ...
    if request.method == "POST" and request.POST.get("action") == "import":
        from customer.models import Customer, GstTreatment
        import re as _re
        ...
        # ── Pre-calculate starting customer code ─────────────────────────────
        all_codes = Customer.objects.values_list("customer_code", flat=True)
        existing_nums = [
            int(n)
        ...
        for entry in deduped_ready:
            ...
            try:
                ...
                exists = Customer.objects.filter(email__iexact=email).first()

                # If existing customer is inactive, treat import as re-activation
                if exists and not exists.is_active:
                    ...
                    obj = exists or Customer()

                # Basic
                obj.email         = email
                obj.customer_type = m.get("customer_type", "individual").lower() or "individual"
                ...
                obj.is_vendor     = to_bool(m.get("is_vendor", ""), default=False)
                ...
                # Auto-generate customer_code only for new records
                if not exists:
                    obj.customer_code = f"CUST-{code_counter:04d}"
                ...
                # If customer is also a vendor, create/update vendor with same details
                if obj.is_vendor:
                    try:
                    ...
                    if Vendor:
                        vendor = Vendor.objects.filter(email__iexact=obj.email).first()
                        if not vendor:
                            vendor = Vendor()

                        vendor.vendor_code = obj.customer_code
                        vendor.vendor_type = obj.customer_type
                        vendor.first_name = obj.first_name
                        vendor.last_name = obj.last_name
                        vendor.company_name = obj.company_name
                        vendor.email = obj.email
                        vendor.phone = obj.phone
                        vendor.mobile = obj.mobile
                        vendor.tax_number = obj.gst_number
                        vendor.address_line_1 = obj.address_line_1
                        vendor.address_line_2 = obj.address_line_2
                        vendor.city = obj.city
                        vendor.postal_code = obj.postal_code
                        vendor.state = obj.state
                        vendor.country = obj.country
                        vendor.shipping_address_line_1 = obj.shipping_address_line_1
                        vendor.shipping_address_line_2 = obj.shipping_address_line_2
                        vendor.shipping_city = obj.shipping_city
                        vendor.shipping_postal_code = obj.shipping_postal_code
                        vendor.shipping_state = obj.shipping_state
                        vendor.shipping_country = obj.shipping_country
                        vendor.pan_number = obj.pan_number
                        vendor.tax_preference = obj.tax_preference
                        vendor.exemption_reason = obj.exemption_reason
                        vendor.currency = obj.currency
                        vendor.payment_terms = obj.payment_terms
                        vendor.opening_balance = obj.opening_balance
                        vendor.gst_treatment = obj.gst_treatment
                        vendor.is_customer = obj.is_vendor
                        vendor.is_active = obj.is_active
                        vendor.created_at = obj.created_at
                        vendor.updated_at = obj.updated_at
                        vendor.created_by = obj.created_by
                        vendor.updated_by = obj.updated_by
                        vendor.save()
        ...
        if error_msgs:
            ...
            messages.success(request,
                f"Import complete — {created_count} customer(s) created, "
                f"{updated_count} updated, {skipped_dup} duplicate(s) skipped.")

        return redirect("customer_list", company_code=company_code)

    return render(request, "customer/customer_import_step3.html", {
        "company_code": company_code,
        "ready":        ready,
        "skipped":      skipped,
        "unmapped":     unmapped,
...
def import_customers_sample(request, company_code):
...
def export_customers_csv(request, company_code):
    from customer.models import Customer
    ...
    qs = (
        Customer.objects.select_related("gst_treatment", "payment_terms")
    ...
    for c in qs:
        ...
        for key in field_keys:
            ...
            if key == "is_active":
                ...
                continue
            if key == "is_vendor":
                row.append(_bool_str(c.is_vendor))
                continue
