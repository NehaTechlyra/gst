...
def get_company_default_warehouse(request=None):
    db = getattr(request, 'company_db', 'default')
    warehouse = Warehouse.objects.using(db).filter(is_default=True).first()
    if not warehouse:
        warehouse = Warehouse.objects.using(db).first()
    return warehouse


def update_stock_from_paid_invoice(inv, user, request):
    """
    Update stock for a sales invoice when payment is completed and stock is managed on payment.
    Uses delivered notes if present, otherwise consumes invoice items from the company's default warehouse.
    """
    ...
    # Apply stock for already delivered notes first.
    ...
    for delivery_note in delivered_notes:
        try:
            ...
            logger.exception(
                'Failed to update stock from delivered SalesDeliveryNote %s',
                getattr(delivery_note, 'delivery_note_number', None)
            )

    if updated:
        # If any delivered notes were updated, we still need to process any remaining
        # invoice quantity that has not been covered by stock-updated delivery notes.
        pass

    warehouse = get_company_default_warehouse(request)
    if not warehouse:
        logger.error(
            'No warehouse found for company db %s when updating stock for paid invoice %s',
            db,
            getattr(inv, 'inv_number', None)
        )
        return False
    ...
    with transaction.atomic():
        for item in inv.items.all():
            ...
            delivered_qty = SalesDeliveryNoteItem.objects.using(db).filter(
                invoice_item=item,
            ...
            remaining_qty = Decimal(item.quantity or 0) - Decimal(delivered_qty or 0)
            if remaining_qty <= 0:
                continue

            stock, _ = Stock.objects.using(db).get_or_create(
                item=product,
                warehouse=warehouse,
                batch_number=None,
                serial_number=None,
...
def quotation_add(request):
    ...
    base_currency_symbol = (base_currency.symbol or base_currency.code or '').strip() if base_currency else ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹'
    ...
    if not base_currency_symbol:
        base_currency_symbol = base_currency.code if base_currency and base_currency.code else ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹'
...
def customer_detail_ajax(request, pk):
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Check if customer has actual shipping details
...
def save_salesquote(request):
    if request.method == "POST":
        ...
        if formset.is_valid():
            ...
            if company_tax_type == 'TURNOVER':
                turnover_rate = Decimal(str(getattr(turnover_tax_obj, 'rate', 0) or 0))
                if turnover_rate:
                    final_total += (final_total * turnover_rate / Decimal('100'))

            tds_tcs_signed_amount = tds_tcs_amount if tds_tcs_type == 'tcs' else -tds_tcs_amount
            final_total += tds_tcs_signed_amount
...
def quotation_detail(request, pk):
    ...
    for item in existing_items_qs:
        ...
        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ NEW: line_total is ONLY the discounted amount (no tax)
        line_total = discounted
        # line_total = discounted + tax_amount

        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ NEW: Subtotal accumulates discounted amounts (before tax)
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ NEW: Total before grand discount = subtotal + tax
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ NEW: Final total = (subtotal + tax) - grand_discount
    final_total = total_before_discount  - grand_discount

    total_discount_combined = total_item_discount + grand_discount
    

    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ NEW: Currency handling for quotations - calculate base currency amounts
    company = _get_company_for_request(request)
    ...
    tds_tcs_type = (quote.tds_tcs_type or '').strip().lower()
    tds_tcs_amount = Decimal(str(quote.tds_tcs_amount or 0))
    ...
    if company_tax_type == 'TURNOVER' and getattr(quote, 'turnover_tax', None):
        turnover_rate = Decimal(str(getattr(quote.turnover_tax, 'rate', 0) or 0))
        if turnover_rate:
            turnover_tax_amount = (final_total * turnover_rate) / Decimal('100')
            turnover_tax_amount_base = (final_total_base * turnover_rate) / Decimal('100')

    tds_tcs_signed_amount_base = tds_tcs_amount_base if tds_tcs_type == 'tcs' else -tds_tcs_amount_base
    tds_tcs_signed_amount = tds_tcs_amount if tds_tcs_type == 'tcs' else -tds_tcs_amount
    final_total += turnover_tax_amount + tds_tcs_signed_amount
    final_total_base += turnover_tax_amount_base + tds_tcs_signed_amount_base
    if final_total < Decimal('0.00'):
        final_total = Decimal('0.00')
    if final_total_base < Decimal('0.00'):
...
def generate_quotation_pdf_bytes(pk, request=None):
    ...
    # Register DejaVu Sans font (supports  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹ symbol)
    try:
        from django.conf import settings
    ...
    currency = context.get('document_currency_symbol') or (" ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹" if font_registered else "Rs.")
    base_currency = context.get('company_base_currency_symbol') or (" ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹" if font_registered else "Rs.")
...
def generate_payment_receipt_pdf_bytes(payment_id):
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ Font registration  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬
    try:
        from django.conf import settings
        static_font_path = os.path.join(settings.BASE_DIR, 'static', 'fonts', 'DejaVuSans.ttf')
        font_registered = False
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ Document setup  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬
    buffer = io.BytesIO()
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ Shared paragraph styles  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ Company info block  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬
    if company:
        company_info = (
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ Meta info (4-column grid)  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬
    customer_name = (
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ Amount highlight box  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬
    amt_table = Table(
        [[
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ Allocations table  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ Notes & signature  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬ ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬
    ...
    if request.method == "POST"and not readonly:
        ...
        if quotation_form.is_valid() and sales_formset.is_valid():
            ...
            for form in sales_formset.forms:
                ...
                item.save()
            if not quote_obj.total_amount_base:
        ...
        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Fetch tax group names in bulk (to avoid N+1 queries)
        ...
        for form, item in zip(sales_formset.forms, existing_items_qs):
            ...
            # #print(f"Checking key {key}  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚ š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’‚¢ƒ¢¢€š¬…¾ƒ€š‚¢ matched UOM: {uom_name}")

            #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Force the dropdown to display the correct text for this specific instance
            if uom_name:
                display_label = f"{item.product.name} ({uom_name})"
            else:
                display_label = item.product.name

            #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Replace the field choices with the selected item label
            form.fields['product'].choices = [
                (item.product.id, display_label)
            ]
            form.initial['product'] = item.product.id
            # #print(f" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Updated dropdown for {display_label}")


        # for item in existing_items_qs:
            # #print("prd_brcd value:", item.prd_brcd, "type:", type(item.prd_brcd))

        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Combine product + UOM + Tax Group for display
        for item in existing_items_qs:
            key = f"{item.product_id}_{item.prd_brcd}"
            uom_name = uoms_by_barcode.get(key, '')
...
#         #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Generate a new quote number using your helper
#         new_quote_number = generate_quote_number()
#         #print("new_quote_number:",new_quote_number)
#         #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Duplicate the quotation
...
#         #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Duplicate all quotation items
...
def order_add(request):
    ...
    try:
        ...
        company_base_currency_symbol = (getattr(base_cur, 'symbol', None) or ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹') if base_cur else ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹'
        company_base_currency_code = getattr(base_cur, 'code', '') if base_cur else ''
    except Exception:
        company_currencies = []
        company_base_currency_symbol = ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹'
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Fetch TDS and TCS for order_add template
...
def order_detail(request, pk):
    ...
    for item in existing_items_qs:
        ...
        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ NEW: Apply discount per unit
        if item.prd_distype == 'percent':
            discount_amount = (base * discount_val) / Decimal('100')
        else:
    ...
    doc_currency_symbol = (getattr(doc_currency, 'symbol', None) or getattr(doc_currency, 'code', None) or ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹') if doc_currency else ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹'
...
def build_order_context(pk, request=None):
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Add company_is_india for conditional tax display
    company_tax_type = _get_request_company_tax_type(request, company)
    turnover_tax_amount = Decimal('0.00')
    turnover_tax_amount_base = Decimal('0.00')
    if company_tax_type == 'TURNOVER':
        ...
        if order_form.is_valid() and sales_formset.is_valid():
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                currency_symbol = (
                    ...
                    or ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹'
                ) if response_currency else ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹'
                return JsonResponse({
                    'success': True,
...
def invoice_detail(request, pk):
    ...
    tds_tcs_type = (inv.tds_tcs_type or '').strip().lower()
    print("tds_tcs_type:", tds_tcs_type)
    tds_tcs_amount = Decimal(str(inv.tds_tcs_amount or 0))
    tds_tcs_amount_base = tds_tcs_amount * fx_rate
    # Use a simple formset to iterate items if needed in template
...
def delete_sales_inv(request, inv_id):
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦by adarshlock CHECK PERIOD LOCK BEFORE DELETING INVOICE
    ...
    try:
        ...
        messages.error(request, error_msg)
        return redirect_with_company('sales_inv_list')
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦by adarshlock
...
def inv_add(request):
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Fetch TDS and TCS for invoice_add template
    tds_tax_master_items = TdsMaster.objects.filter(company=company, is_active=True) if company else TdsMaster.objects.none()
    tcs_tax_master_items = TcsMaster.objects.filter(company=company, is_active=True) if company else TcsMaster.objects.none()
...
def save_salesinvoice(request):
    if request.method == "POST":
        ...
        post_data = request.POST.copy()  # make mutable copy
        prd_brcd_map = {}
        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ CHECK PERIOD LOCK BEFORE CREATING INVOICE
        ...
        if date_str:
            ...
            if invoice_date:
                try:
                    ...
                    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                        return JsonResponse({'success': False, 'error': f" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ¢¢‚¬Å¾‚¢ Cannot create invoice: {str(e)}"}, status=403)
        ...
        try:
            ...
            # Note: do not assign `logger = ...` here  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚ it makes `logger` local for the whole
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({
                ...
                'currency_symbol': invoice.document_currency.symbol if invoice.document_currency else " ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹",
...
def invoice_edit(request, pk):
    ...
    if request.method == "POST" and not readonly:
        ...
        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ CHECK PERIOD LOCK BEFORE EDITING INVOICE
        from datetime import datetime
        from django.core.exceptions import PermissionDenied
        from system_settings.validators import PeriodLockEnforcer
        ...
        try:
            ...
            #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ For AJAX requests, return JSON
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
    ...
    try:
        ...
        company_currencies = Currency.objects.filter(company=company, is_active=True).order_by('code')
        base_currency = company_currencies.filter(is_base=True).first() or company_currencies.first()
        company_base_currency_symbol = (base_currency.symbol or base_currency.code or '').strip() if base_currency else ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹'
    ...
    context = {
        ...
        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Fetch TDS and TCS for invoice_edit template
        'tds_tax_master_items': TdsMaster.objects.filter(company=_get_company_for_request(request), is_active=True),
        'tcs_tax_master_items': TcsMaster.objects.filter(company=_get_company_for_request(request), is_active=True),
    }
    if readonly:
...
def invoice_duplicate(request, pk):
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ CHECK PERIOD LOCK BEFORE DUPLICATING/EDITING INVOICE
    from datetime import datetime
    from django.core.exceptions import PermissionDenied
    from system_settings.validators import PeriodLockEnforcer
    ...
    if invoice_date:
        try:
            ...
            error_msg = f" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ¢¢‚¬Å¾‚¢ Cannot save invoice: {str(e)}"
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({'success': False, 'error': error_msg}, status=403)
            messages.error(request, error_msg)
            return redirect_with_company('sales_inv_list')
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦
...
def duplicate_order(request, pk):
    ...
    if request.method == 'POST':
        ...
        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ FIX 1: Get original INSIDE POST block
        ...
        try:
            ...
            # Clone items -  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ FIX 2: Use new_item.save()
            ...
            for item in items:
                ...
                new_item.save()  #  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚ ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚ WAS: item.save()
            ...
            #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ FIX 3: Redirect to NEW order
...
#                 print(f"DEBUG: Creating allocation for  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{allocation_amount}")
...
#                         f'inv Payment:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{allocation_amount:.2f}, Advance:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{advance_amount:.2f}'
#                     )
#                 else:
#                     messages.success(
#                         request, 
#                         f'Payment #{payment.payment_number} of  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{payment.amount} recorded successfully.'
#                     )
...
def post_payment_journal_entry(payment, request_user):
    ...
    try:
        ...
        print(f"\n ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ¢¢€š¬…€œ DEBIT: {debit_acct.name} -  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{debit_amount:.2f}")
        ...
        print(f" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ¢¢€š¬…€œ CREDIT: {debt_acct.name} -  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{credit_amount:.2f}")
        ...
        print(f"\nTotal Debit:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{journal.total_debit:.2f}")
        print(f"Total Credit:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{journal.total_credit:.2f}")
        print(f"Difference:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{abs(diff):.2f}")
        ...
        if diff != Decimal('0.00'):
            ...
            if round_acct:
                if diff > 0:
                    ...
                    print(f" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ¢¢€š¬…€œ ROUNDING: {round_acct.name} (Credit) -  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{diff:.2f}")
                else:
                    abs_diff = abs(diff)
                    JournalLine.objects.create(
                        journal=journal,
                    ...
                    print(f" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ¢¢€š¬…€œ ROUNDING: {round_acct.name} (Debit) -  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{abs_diff:.2f}")
        ...
        print(f"\n ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ JOURNAL ENTRY POSTED SUCCESSFULLY")
        print(f"Entry Number: {entry_number}")
        print(f"Final Total Debit:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{journal.total_debit:.2f}")
        print(f"Final Total Credit:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{journal.total_credit:.2f}")
        print(f"{'='*80}\n")
        
        return journal
        
    except Exception as e:
        print(f"\n ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ¢¢‚¬Å¾‚¢ ERROR POSTING JOURNAL ENTRY: {str(e)}")
        import traceback
        traceback.print_exc()
...
def save_payment_received(request, pk=None):
    ...
    Scenarios:
    1. No invoices selected  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚ š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’‚¢ƒ¢¢€š¬…¾ƒ€š‚¢ Pure advance payment
    2. No advance + Invoices selected  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚ š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’‚¢ƒ¢¢€š¬…¾ƒ€š‚¢ Direct invoice payment (full or partial)
    3. Advance available + Invoices selected  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚ š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’‚¢ƒ¢¢€š¬…¾ƒ€š‚¢ Use advance first, then cash
    4. Advance insufficient  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚ š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’‚¢ƒ¢¢€š¬…¾ƒ€š‚¢ Use full advance + cash (full or partial payment)
    """
    try:
        ...
        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ CHECK PERIOD LOCK BEFORE CREATING PAYMENT RECEIVED
        from datetime import datetime
        from django.core.exceptions import PermissionDenied
        from system_settings.validators import PeriodLockEnforcer
        ...
        with transaction.atomic():
            ...
            print(f"DEBUG: Current Advance Balance =  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{current_advance}")
            ...
            #by adarsh payment
            # ========================================================================
            # SCENARIO 1: No invoices selected  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚ š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’‚¢ƒ¢¢€š¬…¾ƒ€š‚¢ Pure advance payment
            # ========================================================================
            if not inv_ids:
                print("\n" + "="*60)
                ...
                print(f"DEBUG: Added advance payment of  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{payment_amount}")
                ...
                if journal_entry:
                    print(f" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Journal Entry Posted: {journal_entry.entry_number}")
                else:
                    print(" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’¢‚‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚  ¢‚¬„¢ƒÆ’¢‚‚¯ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¸ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚  Payment saved but journal entry posting encountered an error")
                
                # Use the payment's currency symbol when available (falls back to ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹')
                try:
                    symbol = payment.get_currency_symbol() if hasattr(payment, 'get_currency_symbol') else ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹'
                    if not symbol:
                        symbol = ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹'
                except Exception:
                    symbol = ' ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹'
            ...
            for inv_id in inv_ids:
                ...
                print(f"DEBUG: Invoice #{inv.inv_number} - Total:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{inv.total_amount}, Paid:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{total_paid}, Remaining:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{inv_remaining}")
            ...
            print(f"\nDEBUG: Total Invoices Due =  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{total_invs_due} (across {len(invs_data)} invoice(s))")
            ...
            for inv_data in invs_data:
                ...
                print(f"  - Amount to Pay:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{inv_data['amount_to_pay']}")
                print(f"  - Advance Used:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{inv_data['advance_used']}")
                print(f"  - Cash Needed:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{inv_data['cash_needed']}")
            
            # STEP 2: Determine allocation based on payment sufficiency
            total_cash_needed = sum(id['cash_needed'] for id in invs_data)
            print(f"\nDEBUG: Total Cash Needed =  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{total_cash_needed}, Cash Available =  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{payment_amount}")
            print(f"DEBUG: Total Advance Used =  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{total_advance_used}")
            ...
            if payment_amount < total_cash_needed:
                ...
                if has_advance:
                    ...
                    for inv_data in invs_data:
                        ...
                        print(f"  - Advance Used:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{inv_data['advance_used']:.2f}")
                        print(f"  - Cash Allocated:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{inv_data['cash_allocated']:.2f}")
                        print(f"  - Total Allocated:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{inv_data['total_allocated']:.2f}")
                else:
                ...
                print(f"\nExcess cash for new advance:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{excess_cash:.2f}")
            ...
            for inv_data in invs_data:
                ...
                print(f"  - Allocating:  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{total_allocation:.2f}")
                ...
                # Update invoice status from centralized payment-status logic
                ...
                # of on delivery, update stock when the invoice is fully paid.
                try:
                    if not is_stock_management_on_delivery(request=request) and inv.payment_status_id == 3:
                        update_stock_from_paid_invoice(inv, request.user, request)
                except Exception:
                    logger.exception('Failed to update stock on payment for invoice %s', getattr(inv, 'inv_number', None))
            ...
            if total_advance_used > 0:
                ...
                if advance_debit:
                    print(f" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ¢¢€š¬…€œ Used advance: - ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{total_advance_used:.2f}")
                else:
                    print(f" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€š‚ Failed to use advance (insufficient balance)")
            ...
            if excess_cash > 0:
                ...
                print(f" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ¢¢€š¬…€œ Added new advance: + ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{excess_cash:.2f}")
            ...
            print("="*80)
            print(f"Initial Advance Balance:   ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{current_advance:.2f}")
            print(f"Advance Used for Invoices: - ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{total_advance_used:.2f}")
            print(f"Cash Payment Received:     ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{payment_amount:.2f}")
            print(f"New Advance Added:        + ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{excess_cash:.2f}")
            print(f"Final Advance Balance:     ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{final_balance:.2f}")
...
def update_payment_received(request, payment_id):
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ CHECK PERIOD LOCK BEFORE UPDATING PAYMENT RECEIVED
    from datetime import datetime
    from django.core.exceptions import PermissionDenied
    from system_settings.validators import PeriodLockEnforcer
    ...
    try:
        ...
        if new_selected_inv_ids:
            ...
            if amount < total_invoice_amount - current_advance:
                messages.error(
                    request,
                    f'Payment amount  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{amount} is insufficient. Need at least  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{total_invoice_amount - current_advance} after advance.'
                )
...
def delete_payment_received(request, payment_id):
    ...
    This will:
    1. Reverse advance transactions (debit  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚ š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’‚¢ƒ¢¢€š¬…¾ƒ€š‚¢ credit, credit  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚ š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’‚¢ƒ¢¢€š¬…¾ƒ€š‚¢ debit)
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ CHECK PERIOD LOCK BEFORE DELETING PAYMENT RECEIVED
    from django.core.exceptions import PermissionDenied
    from system_settings.validators import PeriodLockEnforcer
    ...
    try:
        ...
        success_msg = (
            f'Payment #{payment_number} for customer {customer} ( ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹{amount:.2f}) has been deleted successfully. '
    ...
    if request.method == 'POST':
        ...
        if sr.status == 'received':
            ...
            try:
                ...
                if fail_count > 0:
                    messages.error(request, f'{fail_count} stock update/movement operation(s) failed  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚ check server logs.')
...
class SalesReturnUpdateView(UpdateView):
    ...
    def form_invalid(self, form):
        ...
        if hasattr(self.request, 'period_lock_error') and self.request.period_lock_error:
            error_msg = (
                f" ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ¢¢‚¬Å¾‚¢ Period Locked: The fiscal year is locked for editing (sales_return).<br>"
...
def build_performa_context(pk, request=None):
    ...
    for item in existing_items_qs:
        ...
        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ NEW: Base currency calculations anchored to o_price
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ NEW: Calculate base totals manually from items to eliminate rounding drift
...
def performa_inv_add(request):
    ...
    #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Fetch TDS and TCS for performa_invoice_add template
    tds_tax_master_items = TdsMaster.objects.filter(company=company, is_active=True) if company else TdsMaster.objects.none()
    tcs_tax_master_items = TcsMaster.objects.filter(company=company, is_active=True) if company else TcsMaster.objects.none()
...
def performa_invoice_edit(request, pk):
    ...
    return render(request, 'sales/performa_invoice_add.html', {
        ...
        'tds_tcs_rate': invoice.tds_tcs_rate or 0,
        'tds_tcs_amount': invoice.tds_tcs_amount or 0,
        #  ¢‚¬„¢ƒÆ’¢‚‚¢ƒÆ’†€™ƒ¢¢€š¬‚¦ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦¢‚¬Å“š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚¦ Fetch TDS and TCS for performa_invoice_edit template
        'tds_tax_master_items': TdsMaster.objects.filter(company=company, is_active=True),
        'tcs_tax_master_items': TcsMaster.objects.filter(company=company, is_active=True),
    })
...
EWAY_THRESHOLD = 50000  #  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹50,000  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ€¦‚¡ƒÆ’¢‚‚¬š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ¢¢€š¬…€œ GST mandate threshold
...
def eway_bill_generate(request, invoice_id):
    ...
    Only available for Indian companies and invoices  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚‚°ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¥  ¢‚¬„¢ƒÆ’¢‚‚¢š‚¢ƒÆ’‚¢ƒ¢¢€š¬…¡ƒ€š‚¬ƒÆ’¢‚¬¦ƒ€š‚¡ƒÆ’†€™ƒ¢¢€š¬…¡ƒÆ’¢‚‚¹50,000.
    """
