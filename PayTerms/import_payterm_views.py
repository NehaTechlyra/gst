"""
Payment term Import Wizard – import_payterm_views.py
Mapped exactly to your Payment term model fields.
Place in Items/import_payterm_views.py
"""
...
IMPORT_FIELDS = [
    # Basic
    ("name",             "Payment Term Name",              True,  "basic",    "name"),
    ("days",             "Days",                           True,  "basic",    "days"),
]
...
def _validate_rows(rows, mapping):
    ...
    for row in rows:
        ...
        days_raw = mapped.get("days", "").strip()
        if not name:
            errors.append("Payment term Name is required")
        if not days_raw:
            errors.append("Days is required")
        else:
            try:
                days_val = int(days_raw)
                if days_val < 0:
                    errors.append("Days must be a non-negative integer")
            except ValueError:
                errors.append("Days must be a non-negative integer")
...
def import_payterm_step1(request, company_code):
    if request.method == "POST":
        uploaded = request.FILES.get("import_file")
        if not uploaded:
            messages.error(request, "Please select a file.")
            return render(request, "import_payterm_step1.html", {"company_code": company_code})
        ...
        return redirect("import_payterm_step2", company_code=company_code)
...
def import_payterm_step2(request, company_code):
    headers = request.session.get("imp_headers")
    if not headers:
        return redirect("import_payterm_step1", company_code=company_code)
    ...
    if request.method == "POST":
        ...
        for key, *_ in IMPORT_FIELDS:
            ...
            if csv_h:
                mapping[csv_h] = key
        required_keys = {"name", "days"}
        if not required_keys.issubset(set(mapping.values())):
            messages.error(request, "You must map 'Payment Term Name' and 'Days'.")
            # fall through to re-render
        else:
            request.session["imp_mapping"] = mapping
            return redirect("import_payterm_step3", company_code=company_code)

    fields_by_section = get_fields_by_section()

    return render(request, "import_payterm_step2.html", {
...
def import_payterm_step3(request, company_code):
    headers   = request.session.get("imp_headers")
    ...
    if duplicate == "skip" and ready:
        ...
        ready_names = [e["mapped"].get("name", "").strip() for e in ready]
        ready_names = [n for n in ready_names if n]
        if ready_names:
            existing = set(
                PayTerms.objects.annotate(name_l=Lower("name"))
            ...
            if existing:
                next_ready = []
                for entry in ready:
                    name = entry["mapped"].get("name", "").strip()
                    if name and name.lower() in existing:
                        entry = dict(entry)
                        entry["errors"] = entry.get("errors", []) + ["Duplicate payment terms exists (will be skipped)"]
    ...
    if request.method == "POST" and request.POST.get("action") == "import":
        ...
        for entry in ready:
            ...
            try:
                ...
                continue
            try:
                exists = PayTerms.objects.filter(name__iexact=name).first()
        ...
        if error_msgs:
            ...
            messages.success(request,
                f"Import complete - {created_count} payment term(s) created, "
                f"{updated_count} updated, {skipped_dup} duplicate(s) skipped.")

        return redirect_with_company("payterm_view")

    return render(request, "import_payterm_step3.html", {
        "company_code": company_code,
        "ready":        ready,
        "skipped":      skipped,
        "unmapped":     unmapped,
...
def import_payterm_sample(request, company_code):
    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = 'attachment; filename="sample_payterms.csv"'
