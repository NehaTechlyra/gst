"""
Warehouse Import Wizard – import_warehouse_views.py
Mapped exactly to your Warehouse model fields.
Place in Items/import_warehouse_views.py
"""
...
IMPORT_FIELDS = [
    # Basic
    ("warehouse_name",      "Warehouse Name",      True,  "basic", "warehouse_name"),
    ("code",                "Code",                False, "basic", "code"),
    ("address",             "Address",             False, "basic", "address"),
    ("contact_phone",       "Contact Phone",       False, "basic", "contact_phone"),
    ("contact_email",       "Contact Email",       False, "basic", "contact_email"),
    ("description",         "Description",         False, "basic", "description"),
    ("warehouse_incharge",  "Warehouse Incharge",  False, "basic", "warehouse_incharge"),
    ("is_default",          "Is Default",          False, "basic", None),
]
...
def _validate_rows(rows, mapping):
    ...
    for row in rows:
        ...
        errors = []
        name = mapped.get("warehouse_name", "").strip()
        if not name:
...
def import_warehouse_step1(request, company_code):
    if request.method == "POST":
        uploaded = request.FILES.get("import_file")
        if not uploaded:
            messages.error(request, "Please select a file.")
            return render(request, "import_warehouse_step1.html", {"company_code": company_code})
        ...
        return redirect("import_warehouse_step2", company_code=company_code)
...
def import_warehouse_step2(request, company_code):
    headers = request.session.get("imp_headers")
    if not headers:
        return redirect("import_warehouse_step1", company_code=company_code)
    ...
    if request.method == "POST":
        ...
        for key, *_ in IMPORT_FIELDS:
            ...
            if csv_h:
                mapping[csv_h] = key
        if not any(v == "warehouse_name" for v in mapping.values()):
            messages.error(request, "You must map at least the 'Warehouse Name' column.")
            # fall through to re-render
        else:
            request.session["imp_mapping"] = mapping
            return redirect("import_warehouse_step3", company_code=company_code)

    fields_by_section = get_fields_by_section()

    return render(request, "import_warehouse_step2.html", {
...
def import_warehouse_step3(request, company_code):
    headers   = request.session.get("imp_headers")
    ...
    if duplicate == "skip" and ready:
        ...
        ready_names = [e["mapped"].get("warehouse_name", "").strip() for e in ready]
        ready_names = [n for n in ready_names if n]
        if ready_names:
            existing = set(
                Warehouse.objects.annotate(name_l=Lower("warehouse_name"))
            ...
            if existing:
                next_ready = []
                for entry in ready:
                    name = entry["mapped"].get("warehouse_name", "").strip()
                    if name and name.lower() in existing:
                        entry = dict(entry)
                        entry["errors"] = entry.get("errors", []) + ["Duplicate warehouse exists (will be skipped)"]
    ...
    if request.method == "POST" and request.POST.get("action") == "import":
        ...
        def set_if_mapped(obj, m, field_name):
            if field_name in m:
                setattr(obj, field_name, m.get(field_name, "").strip())

        for entry in ready:
            m    = entry["mapped"]
            name = m.get("warehouse_name", "").strip()
            if not name:
                continue
            try:
                exists = Warehouse.objects.filter(warehouse_name__iexact=name).first()
                ...
                obj = exists or Warehouse()
                obj.warehouse_name = name
                set_if_mapped(obj, m, "code")
                set_if_mapped(obj, m, "address")
                set_if_mapped(obj, m, "contact_phone")
                set_if_mapped(obj, m, "contact_email")
                set_if_mapped(obj, m, "description")
                set_if_mapped(obj, m, "warehouse_incharge")
                if "is_default" in m:
                    obj.is_default = to_bool(m.get("is_default", ""))
                obj.status = True
        ...
        if error_msgs:
            ...
            messages.success(request,
                f"Import complete - {created_count} warehouse(s) created, "
                f"{updated_count} updated, {skipped_dup} duplicate(s) skipped.")

        return redirect_with_company("warehouse_list")

    return render(request, "import_warehouse_step3.html", {
        "company_code": company_code,
        "ready":        ready,
        "skipped":      skipped,
        "unmapped":     unmapped,
...
def import_warehouse_sample(request, company_code):
    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = 'attachment; filename="sample_warehouses.csv"'
    writer = csv.writer(response)
    writer.writerow([label for _, label, *_ in IMPORT_FIELDS])
    writer.writerow([
        "Central Warehouse",
        "WH-001",
        "12 Industrial Estate, Phase 1, Bengaluru",
        "+91-98765-43210",
        "central@example.com",
    ...
    writer.writerow([
        "North Depot",
        "WH-002",
        "45 Ring Road, New Delhi",
        "+91-91234-56789",
        "north@example.com",
        "Overflow storage for seasonal stock",
        "Priya Shah",
        "no",
    ])
    return response
