"""
Unit Import Wizard – import_unit_views.py
Mapped exactly to your Unit model fields.
Place in Items/import_unit_views.py
"""
...
IMPORT_FIELDS = [
    # Basic
    ("unit_name",             "Unit Name",              True,  "basic",    "unit_name"),
]
...
def _validate_rows(rows, mapping):
    ...
    for row in rows:
        ...
        errors = []
        name = mapped.get("unit_name", "").strip()
        if not name:
...
def import_unit_step1(request, company_code):
    if request.method == "POST":
        uploaded = request.FILES.get("import_file")
        if not uploaded:
            messages.error(request, "Please select a file.")
            return render(request, "import_unit_step1.html", {"company_code": company_code})
        ...
        return redirect("import_unit_step2", company_code=company_code)
...
def import_unit_step2(request, company_code):
    headers = request.session.get("imp_headers")
    if not headers:
        return redirect("import_unit_step1", company_code=company_code)
    ...
    if request.method == "POST":
        ...
        for key, *_ in IMPORT_FIELDS:
            ...
            if csv_h:
                mapping[csv_h] = key
        if not any(v == "unit_name" for v in mapping.values()):
            messages.error(request, "You must map at least the 'Unit Name' column.")
            # fall through to re-render
        else:
            request.session["imp_mapping"] = mapping
            return redirect("import_unit_step3", company_code=company_code)

    fields_by_section = get_fields_by_section()

    return render(request, "import_unit_step2.html", {
...
def import_unit_step3(request, company_code):
    headers   = request.session.get("imp_headers")
    ...
    if duplicate == "skip" and ready:
        ...
        ready_names = [e["mapped"].get("unit_name", "").strip() for e in ready]
        ready_names = [n for n in ready_names if n]
        if ready_names:
            existing = set(
                Unit.objects.annotate(name_l=Lower("unit_name"))
            ...
            if existing:
                next_ready = []
                for entry in ready:
                    name = entry["mapped"].get("unit_name", "").strip()
                    if name and name.lower() in existing:
                        entry = dict(entry)
                        entry["errors"] = entry.get("errors", []) + ["Duplicate unit exists (will be skipped)"]
    ...
    if request.method == "POST" and request.POST.get("action") == "import":
        ...
        for entry in ready:
            m    = entry["mapped"]
            name = m.get("unit_name", "").strip()
            if not name:
                continue
            try:
                exists = Unit.objects.filter(unit_name__iexact=name).first()
...
def import_unit_sample(request, company_code):
    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = 'attachment; filename="sample_unit.csv"'
