"""
Category Import Wizard – import_category_views.py
Mapped exactly to your Category model fields.
Place in Items/import_category_views.py
"""
...
IMPORT_FIELDS = [
    # Basic
    ("category_name",             "Category Name",              True,  "basic",    "category_name"),
]
...
def _validate_rows(rows, mapping):
    ...
    for row in rows:
        ...
        errors = []
        name = mapped.get("category_name", "").strip()
        if not name:
...
def import_category_step1(request, company_code):
    if request.method == "POST":
        uploaded = request.FILES.get("import_file")
        if not uploaded:
            messages.error(request, "Please select a file.")
            return render(request, "import_category_step1.html", {"company_code": company_code})
        ...
        return redirect("import_category_step2", company_code=company_code)
...
def import_category_step2(request, company_code):
    headers = request.session.get("imp_headers")
    if not headers:
        return redirect("import_category_step1", company_code=company_code)
    ...
    if request.method == "POST":
        ...
        for key, *_ in IMPORT_FIELDS:
            ...
            if csv_h:
                mapping[csv_h] = key
        if not any(v == "category_name" for v in mapping.values()):
            messages.error(request, "You must map at least the 'Category Name' column.")
            # fall through to re-render
        else:
            request.session["imp_mapping"] = mapping
            return redirect("import_category_step3", company_code=company_code)

    fields_by_section = get_fields_by_section()

    return render(request, "import_category_step2.html", {
...
def import_category_step3(request, company_code):
    headers   = request.session.get("imp_headers")
    ...
    if duplicate == "skip" and ready:
        ...
        ready_names = [e["mapped"].get("category_name", "").strip() for e in ready]
        ready_names = [n for n in ready_names if n]
        if ready_names:
            existing = set(
                Category.objects.annotate(name_l=Lower("category_name"))
            ...
            if existing:
                next_ready = []
                for entry in ready:
                    name = entry["mapped"].get("category_name", "").strip()
                    if name and name.lower() in existing:
                        entry = dict(entry)
                        entry["errors"] = entry.get("errors", []) + ["Duplicate category exists (will be skipped)"]
    ...
    if request.method == "POST" and request.POST.get("action") == "import":
        ...
        for entry in ready:
            m    = entry["mapped"]
            name = m.get("category_name", "").strip()
            if not name:
                continue
            try:
                exists = Category.objects.filter(category_name__iexact=name).first()
        ...
        if error_msgs:
            ...
            messages.success(request,
                f"Import complete - {created_count} category(s) created, "
                f"{updated_count} updated, {skipped_dup} duplicate(s) skipped.")

        return redirect_with_company("category_list")

    return render(request, "import_category_step3.html", {
        "company_code": company_code,
        "ready":        ready,
        "skipped":      skipped,
        "unmapped":     unmapped,
...
def import_category_sample(request, company_code):
    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = 'attachment; filename="sample_category.csv"'
