"""
Brand Import Wizard – import_brand_views.py
Mapped exactly to your Brand model fields.
Place in Items/import_brand_views.py
"""
...
IMPORT_FIELDS = [
    # Basic
    ("brand_name",             "Brand Name",              True,  "basic",    "brand_name"),
]
...
def _validate_rows(rows, mapping):
    ...
    for row in rows:
        ...
        errors = []
        name = mapped.get("brand_name", "").strip()
        if not name:
...
def import_brand_step1(request, company_code):
    if request.method == "POST":
        uploaded = request.FILES.get("import_file")
        if not uploaded:
            messages.error(request, "Please select a file.")
            return render(request, "import_brand_step1.html", {"company_code": company_code})
        ...
        return redirect("import_brand_step2", company_code=company_code)
...
def import_brand_step2(request, company_code):
    headers = request.session.get("imp_headers")
    if not headers:
        return redirect("import_brand_step1", company_code=company_code)
    ...
    if request.method == "POST":
        ...
        for key, *_ in IMPORT_FIELDS:
            ...
            if csv_h:
                mapping[csv_h] = key
        if not any(v == "brand_name" for v in mapping.values()):
            messages.error(request, "You must map at least the 'Brand Name' column.")
            # fall through to re-render
        else:
            request.session["imp_mapping"] = mapping
            return redirect("import_brand_step3", company_code=company_code)

    fields_by_section = get_fields_by_section()

    return render(request, "import_brand_step2.html", {
...
def import_brand_step3(request, company_code):
    headers   = request.session.get("imp_headers")
    ...
    if duplicate == "skip" and ready:
        ...
        ready_names = [e["mapped"].get("brand_name", "").strip() for e in ready]
        ready_names = [n for n in ready_names if n]
        if ready_names:
            existing = set(
                Brand.objects.annotate(name_l=Lower("brand_name"))
            ...
            if existing:
                next_ready = []
                for entry in ready:
                    name = entry["mapped"].get("brand_name", "").strip()
                    if name and name.lower() in existing:
                        entry = dict(entry)
                        entry["errors"] = entry.get("errors", []) + ["Duplicate brand exists (will be skipped)"]
    ...
    if request.method == "POST" and request.POST.get("action") == "import":
        ...
        for entry in ready:
            ...
            if not name:
                continue
            try:
                exists = Brand.objects.filter(brand_name__iexact=name).first()
...
def import_brand_sample(request, company_code):
    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = 'attachment; filename="sample_brands.csv"'
